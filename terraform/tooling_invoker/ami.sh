...
##############################################
# TEMPORARY: Download JavaScript Test Runner #
##############################################

CONTAINER_DIR="/opt/containers/javascript-test-runner/releases/latest"
echo $CONTAINER_DIR
mkdir -p $CONTAINER_DIR
...
pushd $CONTAINER_DIR
  ...
  /opt/container_tools/img pull -state /tmp/state-img 591712695352.dkr.ecr.eu-west-2.amazonaws.com/javascript-test-runner:latest
  /opt/container_tools/img unpack -state /tmp/state-img 591712695352.dkr.ecr.eu-west-2.amazonaws.com/javascript-test-runner:latest
  chmod -R a-w rootfs
  chmod -R go-rwx rootfs
popd

ln -s $CONTAINER_DIR /opt/containers/javascript-test-runner/current
